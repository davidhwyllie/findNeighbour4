...
class preComparer:
    ...
    def __init__(self, selection_cutoff, over_selection_cutoff_ignore_factor, uncertain_base, catWalk_parameters={}, **kwargs):
        ...
        if self.catWalk is None:  # no catwalk
            logging.info(
                "Catwalk is not running, using python-based set operations with uncertain bases representing {0}".format(
                    self.uncertain_base
                )
            )
    ...
    def _refresh(self):
        ...
        self.composition = (
            {}
        )  # composition statistics for each sequence. Computed and stored on loading for both catWalk and python comparison engines.
    ...
    def mcompare(self, guid, guids=None):
        ...
        ):  # seqProfile keys do include all the samples stored in catWalk + the invalid ones; what we're asked for should be in here
            raise KeyError(
                "Asked to compare {0}  but guid requested has not been stored.  call .persist() on the sample to be added before using mcompare."
            )
    ...
    def compare(self, key1, key2):
        ...
        res = {"guid1": key1, "guid2": key2, "dist": None}
