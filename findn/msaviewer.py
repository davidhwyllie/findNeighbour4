...
class DepictMSA:
    ...
    def __init__(
        ...
        self.df.sort_values(
            by=["p_value3", "A", "C", "G", "T"], inplace=True, ascending=False
        )  # sort by mixture detection statistic
        ...
        if self.iupac is None:
            if (
                len(self.composition.keys() - valid_characters[False]) > 0
            ):  # there are iupac, or other disallowed characters in the sequences
                self.iupac = True
            else:
                self.iupac = False
    ...
    def render_msa(self, cluster_name=None):
        ...
        info = Div(
            text="Cluster {0} with {1} sequences and {2} variant bases in alignment. ".format(
                cluster_name, len(self.df.index), self.align_width
            )
        )
        ...
        if self.nSeqs * self.align_width > self.max_elements_in_plot:
            d = Div(
                text="The alignment is too large to show with {0} elements.  A maximum of {1} elements are allowed.  The cutoff, which is in place for performance reasons, can be altered by setting max_elements_in_plot when constructing a MSAViewer object.".format(
                    self.nSeqs * self.align_width, self.max_elements_in_plot
                )
            )
            ...
            select = figure(
                title="Drag the middle and edges of the selection box to see multisequence alignment",
                plot_height=self.nSeqs + 100,
                plot_width=1200,
                y_range=p.y_range,
                y_axis_type=None,
                x_axis_type=None,
            ...
            if self.qq is not None:
                if len(self.qq.index) > 0:
                    ...
                    qq_plot = figure(
                        width=1200,
                        height=200,
                        title="QQ plot: non linearity is evidence for non-random distribution of variants. Yellow=expected, Blue= observed.",
                    )
