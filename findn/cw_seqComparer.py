...
class cw_seqComparer:
    ...
    def verify_insertion(self, guid):
        ...
        Present in catwalk but not database --> delete from catwalk, return 0
        ...
        if exists_in_catwalk and exists_in_database:
            break       # go to next section
        elif exists_in_catwalk == False and exists_in_database == False:
            return 0
        elif exists_in_catwalk == True and exists_in_database == False:
            # delete from catwalk
            self.catWalk.remove_sample(guid)
            return 0
        elif exists_in_catwalk == False and exists_in_database == True:
            # load from database, add to catwalk
            self.repoopulate(guid)
        ...
        # make a set containing registered pairs, both ways round
        registered_pairs1 = set( ["{0}|{1}".format(guid,guid2) for guid2 in registered_links] )
        for guid2 in registered_links:
            registered_pairs.add("{0}|{1}".format(guid2,guid))
        registered_links =set(registered_links)
        ...
        for i, item in enumerate(mcompare_result["neighbours"]):  # all against all
            ...
            if not guid1 == guid2:
                link = {"dist": dist}
            if dist is not None:
                if guid1 in registered_links:
                    if not "{0}|{1}".format(guid1, guid2) in registered_pairs:
                    links[guid2] = link
