...
""",
    ...
    if not recompress_subset == "abcdef0123456789":
        recompress_subset_label = "recompress_subset_{0}_to_{1}".format(
            recompress_subset_chars[0], recompress_subset_chars[-1]
        )
    ...
    while True:
        ...
        to_update = PERSIST.singletons(
            method="exact", return_top=100
        )  # compress in small batches of 100, after which we will reassess what is worst.

        logger.info(
            "(instance = {1}) | Found records which may need compression; examining {0} with higher singleton estimates.".format(
                len(to_update.index), recompress_subset_label
            )
        )
        ...
        if nModified == 0:
            ...
            logger.info(
                "(instance = {0}) | Nothing found to repack within the specimen number range for this dbmonitor.  Waiting 1 min .. ".format(
                    recompress_subset_label
                )
            )
