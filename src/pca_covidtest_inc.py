...
Example usage:
    ...
    # CONFIG is a findNeighbour4 json config object, loaded from file
    # PERSIST is a findNeighbour4 persistence object, allowing connection to mongo database underlying a (possibly active) findNeighbour4 server.
    builder = ModelBuilder(CONFIG, PERSIST)
   
    print("Building model")
    vm = builder.build(train_on=5000, n_components=10, pca_parameters= {})       # to run on all samples, pass None and 100   
    
    print("Exporting sqlite")
    fn = vm.to_sqlite()

    print("Exporting excel (often slow)")
...
class VariationModel():
    ...
    def serialise(self):
        ...
        serialised = {}
        for key in vm.model:
            if not key == 'vmodel':                 # contains all variant sites for all isolates - really huge
    ...
    def build(self, n_components, pca_parameters={}, min_variant_freq=None, train_on = None, deterministic = True):
        ...
        for i,row in enumerate(pca.components_,0):
            ...
            for j,cell in enumerate(row,0):
                ...
                contribs.append({'pc':i, 'pos':pos, 'allele': allele,'col':vmodel.columns[j], 'weight':cell, 'outside_3mad':outside_3mad})
        ...
        self.vm.add('explained_variance_ratio', list(pca.explained_variance_ratio_))
