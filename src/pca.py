#!/usr/bin/env python
""" 
A component of a findNeighbour3 server which provide relatedness information for bacterial genomes.
...
class pcaComparer():    
    ...
    * compare and mcompare - compare pairs, or all, samples, respectively.
    ...
    def compress(self, guid, obj, enable_bar=True):
        ...
        if use_bar:
            print("mcompress: Computing eigenvalues .. applying missing data inflation parameter ..")

        mix_quality_info = pd.DataFrame.from_dict(missing, orient='index')
        tmp = eigenvalues.merge(mix_quality_info, left_index=True, right_index=True, how='inner')
        eigenvalues = eigenvalues.multiply(tmp['N_eigenvalue_inflation_parameter'], axis = 0)
        if use_bar:
            print("mcompress: Computing eigenvalues .. finished")
...
""")
    ...
    model = builder.build(train_on=5000, n_components=100)
    what = ['all','ACTG']
    compare_with_SNV = True
    pcv = pcaValidator(CONFIG= CONFIG, PERSIST=PERSIST, model=model, what = what    )
    
    pca_ceiling = pcv.validate(sample_size=200, internal=True, compare_with_SNV=compare_with_SNV)
    for item in what:
