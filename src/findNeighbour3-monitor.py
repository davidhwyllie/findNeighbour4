...
class depict_server_status():
    """ depicts server status """
                ...
                for item in self.monitoring_data['recent_server'].index:
                    for column_name in self.monitoring_data['recent_server'].columns.values.tolist():
                        if column_name.startswith('server|'):
                            data.append([self.make_human_readable(column_name),self.monitoring_data['recent_server'].loc[item,column_name]])
                        if column_name=='t':
    ...
    def __init__(self,
                 tmpdir=os.path.join("..","unittest_tmp"),
                 logfile=None,
                 server_url='Not specified',
                 server_port='Not specified',
                 server_description='Not specified'
                 ):
        """ initialises depict_server_status object
        
        input:
        tmpdir:     a writable directory to which information can be written
        ...
        if not os.path.exists(self.tmpdir):
            raise FileNotFoundError("Directory specified {0} does not exist".format(self.tmpdir))
        self.source = {}
        self.source_columns={}
        ...
        self.tag2readable = {
            ...
            "server|mstat|":"findNeighbour3 server RAM ",
            "server|scstat|n":"findNeighbour3 server in-RAM sequences, ",
            "interval":"Interval between events (seconds)",
            "dstats|guid2neighbour|": "Backend MongoDb database, guid2neighbour collection (stores similar sequence pairs), ",
            "dstats|guid2meta|": "Backend MongoDb database, guid2metadata collection (stores meta data about sequences), ",
            "dstats|server_monitoring|": "Backend MongoDb database, server monitoring collection (stores server state information over time), ",
    ...
    def make_human_readable(self, column_header):
    ...
    def depict(self, data_tag='all', tab_title="No title", metrics = None, colorpoints=None, x_axis_label = 'Order status recorded (Oldest --> Most recent)'):
        ...
        for item in report_columns:                        # for the metrics we're supposed to start with
            if item in self.monitoring_data[data_tag].columns.tolist():       # it's a valid column
                ...
                if len(nnan)>0:
                    # plot by event 
                    single_plots['order'][item] = figure(plot_height=150, title= self.make_human_readable(item), plot_width=1000, tools=tools, tooltips=TOOLTIPS)
                    single_plots['order'][item].xaxis.axis_label=''
    ...
    def make_report(self, all_result, recent_result):
        ...
        doc4 = Tabs(tabs=[self.server_info_tab(), tab_rserver,tab_rmemory,tab_server, tab_memory, tab_g2n, tab_g2m, tab_sm, tab_log])

        return doc4

# unittests
class test_init(unittest.TestCase):
    """ tests init method of depict_server_status """
    def runTest(self):
        """ tests init """
        
        dss = depict_server_status()
...
class test_depict_1(unittest.TestCase):
    def runTest(self):
        ...
        dss = depict_server_status(logfile= logfile)
...
if __name__ == '__main__':
        ...
        try:
             ...
             if e.__module__ == "pymongo.errors":
                 ...
                 raise
        dss1 = depict_server_status(logfile= CONFIG['LOGFILE'],
        ...
        while True:
            ...
            html = file_html(page_content, CDN, "fn3_rpt_v1")
            PERSIST.monitor_store('fn3monitor_report', html)
