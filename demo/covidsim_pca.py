...
if __name__ == "__main__":
    ...
    for fastafile in fastafiles:
        ...
        cogfile_df = cogfile_df.drop(columns=['A','C','G','T','is_variant','record_id'])
        ...
        for addition_date in addition_dates:
            ...
            if n_added > 200:
                ...
                sqlite_file = "{0}/{1}.sqlite".format(
                                args.outputdir,
                                fastastem + "_" + str(addition_date)
                                )
                if os.path.exists(sqlite_file):
                    os.unlink(sqlite_file)

                sqlitedb = "sqlite:///{0}".format(
                                sqlite_file
                                )
                ...
                destfile = os.path.join(args.outputdir, fastastem + "_" + addition_date.isoformat() + ".meta.txt")
                this_meta = cogfile_df[cogfile_df['sample_date'] <= addition_date]
                with open(destfile, "wt") as f:
                    this_meta.to_csv(f, date_format="YYYY-MM-DD", index=False, sep=",")
                logging.info("Wrote {0} rows of metadata to {1}".format(len(this_meta.index), destfile))
                ...
                logging.info("Building contingency tables, relating Lineage to pc_cat for {0} days to {1}".format(
                ...
                sig_trends = pdm.significant_tests_performed(build_int_id = lbii)       # by default, uses p=0.01 and negative binomial model output
                
                destfile = os.path.join(args.outputdir, fastastem + "_" + addition_date.isoformat() + ".sigtrends.txt")
                with open(destfile, "wt") as f:
                    sig_trends.to_csv(f, date_format="YYYY-MM-DD", index=False, sep=",")
              
                features = pdm.feature_association(feature = 'pangolearn:lineage_1')
                destfile = os.path.join(args.outputdir, fastastem + "_" + addition_date.isoformat() + ".assocs.txt")
                with open(destfile, "wt") as f:
