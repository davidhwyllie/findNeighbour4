...
for (dbfile in sort(Sys.glob(globpath), decreasing=FALSE)){
  ...
  sigtest_sql = "select ps.pop_int_id, ps.level_1_category_type, ps.level_1_category, ps.level_2_category_type, ps.level_2_category, pcas.pcas_int_id, pcas.pc_cat, pcas.earliest_date, pcas.latest_date,pcas.n, smf.statmodel_int_id, sm.analysis_type, smf.estimate, 
  smf.estimate_ci_high, smf.estimate_ci_low, smf.p_value, sm.analysis_type from population_studied ps inner join pca_summary pcas on pcas.pop_int_id = ps.pop_int_id inner join statistical_model sm on sm.pcas_int_id = pcas.pcas_int_id inner join statistical_model_fit smf
  ...
  tests <- subset(tests, analysis_type == "GLM:Poisson regression" )
  tests$adj_p_value <- p.adjust(tests$p_value, method = 'fdr')
  tests <- subset(tests, adj_p_value < 0.05)
 
  # export any positive trends
  md_sql = "select * from modelled_data"
  md <- database_query(db_conn, md_sql)

  tests <- subset(tests, adj_p_value < 0.001)

  testfile <- file.path(inputdir,gsub('.sqlite','.sigtrends.txt',basename(dbfile)))
  assocfile <- file.path(inputdir, gsub('.sqlite','.assocs.txt',basename(dbfile)))
 
  false_positives <- subset(tests, level_2_category == 'lineage_0')
  intralineage_positives <- subset(tests, level_2_category == 'lineage_1')
  signal <- subset(tests, level_2_category == 'UKREGION')
  assocs <- read.csv(assocfile, header= TRUE)
  lin1_assocs_10 <- subset(assocs, sequencefeature == 'pangolearn:lineage_1' & log_or > 1 & a > 0 ) # OR at least 100

  signal1 <- subset(signal, pc_cat %in% lin1_assocs_10$pc_cat & n >= 3) 
 
 
  # BH method p_value control applied
  print(nrow(tests))
  print(nrow(subset(tests, adj_p_value < 0.01)))
  print(nrow(subset(tests, adj_p_value < 0.003)))
  print(nrow(subset(tests, adj_p_value < 0.001)))
  print(nrow(signal))
  print(nrow(signal1))
  if (nrow(signal)>0){
    #print(signal)
  ...
  if (db_id == 1) {
    ...
    outputfile <- file.path(outputdir, paste0(as.character(sim_id),'Figure2_cut_tcc_example_',as.character(analysis_date), "_", db_id,'.svg'))
...
outputfile <- file.path(outputdir, paste0(sim_id, '_Figure3_TCC_characteristics.Rds'))
print(paste("Wrote data file to ",outputfile))
saveRDS(res_df, outputfile)

outputfile <- file.path(outputdir, paste0(sim_id, '_Figure3_TCC_characteristics.csv'))
write.csv(res_df, outputfile, row.names = FALSE)

outputfile <- file.path(outputdir, paste0(sim_id, '_Figure4_Detections.Rds'))
detect_df <- as.data.table(do.call(rbind, detections))
print(paste("Wrote data file to ",outputfile))
saveRDS(detect_df, outputfile)


outputfile <- file.path(outputdir, paste0(sim_id, '_Figure4_Detections.csv'))
...
outputfile <- file.path(outputdir, paste0(sim_id, '_Figure3_TCC_characteristics.svg'))
...
outputfile <- file.path(outputdir, paste0(sim_id, '_Figure4_Detections.svg'))
