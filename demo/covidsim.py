...
if __name__ == "__main__":
    ...
    if config_file is None:
            ...
            exit(
                f"No config file name supplied."
            )
    ...
    fn4c = fn4Client(server_url)  # expects operation on local host; pass baseurl if somewhere else.
    ...
    for fastafile in fastafiles:
        ...
        for record in Bio.SeqIO.parse(fastafile, "fasta"):
            ...
            res = {"record_id": record.id, "sample_date":sample_date, "guid": sample_id, "seqlen": len(seq),  **counter, "is_variant":is_variant}
            records.append(res)
           
        logger.info(
            "Complete read of simulated data into RAM.  There are {0} sequences.  Sorting ...".format(len(records))
                   )
        ...
        for addition_date in addition_dates:
            ...
            if n_added > 200:
                ...
                try:
                    ...
                    vm.to_sqlite(outputdir=args.outputdir, analysis_name=fastastem+"_"+str(addition_date))
                except ValueError:
                    warnings.warn("Failed to run PCA - usually reflects inadequate amount of data")
