...
""",
    ...
    parser.add_argument(
        "--select_from_pca_output_file",
        help="selects samples from an sqlite database, which contains details of good quality samples.  Will write a guidetree table into the database containing samples selected. If missing, will select based on --sample_quality_threshold",
        action="store",
    )
    parser.add_argument(
        "--sample_quality_cutoff",
        help="Only include in the tree samples with >= this proportion of ACTG bases",
        action="store",
    )
    parser.add_argument(
        "--snp_cutoff",
        help="Find representatives for tree building > snp_cutoff apart.  If omitted, maximum distance stored in the relatedness server is used",
        action="store",
    )
    parser.add_argument(
        "--max_neighbourhood_mixpore",
        help="exclude samples failing mixpore test on at most max_neighbourhood_mixpore closely related samples.  If omitted, mixpore check is not done",
        action="store",
    )
    parser.add_argument(
        "--root_sample",
        help="regard the closest isolate to this sample as the root.  The root_sample can be ancestral (e.g. Wuhan reference, or the inferred TB Ancester (I Comas 2013)), or it can be an outgroup.  A tree will be generated without this sample.",
        action="store",
    )
    parser.add_argument(
        "--output_filestem",
        help="the first part of the output file basename.  e.g. if you want the output file called my_tree.nwk, enter my_tree",
        action="store",
    )
    ...
    if args.select_from_pca_output_file is not None:
        ...
        if not args.select_from_pca_output_file.endswith(".sqlite"):
            raise FileNotFoundError(
                "The file {0} exists but it does not end with .sqlite".format(args.select_from_pca_output_file)
            )
        ...
        if len(extra_samples_in_pca_only) > 0:
            raise KeyError(
                "The PCA sqlite file is incompatible with the server data; the PCA contains {0} samples  not present in the server".froamt9len(
                    extra_samples_in_pca_only
                )
            )
        ...
        logging.info(
            "No SQLite file containing PCA output provided.  The tree will be built from samples in the relatedness server."
        )
    ...
    if args.snp_cutoff is not None:
        if args.snp_cutoff >= 0 and args.snp_cutoff <= CONFIG["SNPCEILING"]:
            ...
            raise ValueError(
                "Invalid snp_cutoff: must be either omitted; or 0 or more; or less than {0}.  It was {1}".format(
                    CONFIG["SNPCEILING"], args.snp_cutoff
                )
            )
    else:
    ...
    if args.sample_quality_cutoff is not None:
        if args.sample_quality_cutoff >= 0 and args.sample_quality_cutoff <= 1:
            ...
            raise ValueError(
                "Invalid snp_cutoff: must be either omitted; or 0 to 1 inclusive.  Config file states {0}, passed was {1}".format(
                    CONFIG["SNPCEILING"], args.sample_quality_cutoff
                )
            )
    else:
    ...
    high_quality_samples = high_quality_samples[
        high_quality_samples.index.isin(all_samples)
    ]  # subselect those within the PCA if appropriate
    sampling_population = high_quality_samples.index.to_list()
    sampling_population = set(sampling_population)  # need rapid search
    logging.info(
        "From all {0} eligible samples currently present, studying {1} high quality (ACTG > 0.9) samples ".format(
            len(sample_annotations.index.to_list()), len(sampling_population)
        )
    )
    ...
    while len(remaining_samples) > 0:
        ...
        test_sample_neighbour_info = PERSIST.guid2neighbours(test_sample, snp_cutoff, returned_format=1)  
        ...
        for neighbour, dist in test_sample_neighbours:
            representatives.append(
                {"representative": test_sample, "neighbour": neighbour, "distance": dist, "is_singleton": is_singleton}
            )
    ...
    logging.info(
        "Generating fasta output from {0} samples; ignoring {1} singletons.".format(
            len(representative_subsample), len(singletons)
        )
    )
    ...
    if os.environ.get("FASTTREE_DIR") is not None:
        single_cmd = "{0}/FastTreeMP -fastest -gtr -nopr -nt {1} > {2} 2> {3}".format(
            os.environ.get("FASTTREE_DIR"), fasta_outputfile, nwk_outputfile, meta_outputfile
        )
        subprocess_cmd = [
        ...
        logging.error(
            "No tree built as FASTTREE environment variable not found.  put it in .env if using a virtual environment, and point it to the directory containing the fasttreeMP executable.  The fasttreeMP should be complied with double precision flags, see fastTree docs."
        )
