...
python fn4_pca.py demos/covid/covid_config_v3.json  # all samples, 200 pcs, output to default location
...
python fn4_pca.py ../config/myConfigFile.json  --outputdir /data/data/pca --analysis_name 2021-04-08 --train_on 50 --n_components 5

### TODO: specify that the code only runs on a subset of the data available
...
""")
    ...
    parser.add_argument('--outputdir', type=str, action='store', nargs='?',
                        help='the directory in which the output will appear', default='')
    parser.add_argument('--analysis_name', type=str, action='store', nargs='?',
                        help='the name of the analysis.  used as the stem of the .sqlite file into which the output is written.', default='pca_output')
    parser.add_argument('--num_train_on', type=int, action='store', nargs='?',
                        help='the number of samples to train on.  Omitting this parameter will run an analysis on the whole dataset.', default=None)
    parser.add_argument('--n_components', type=int, action='store', nargs='?',
                        help='the number of pcs to extract.  If this is smaller than the numbers of samples trained_on, instability is likely.  Default 200', default=200)
