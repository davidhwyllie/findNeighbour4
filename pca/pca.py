...
class TestPersistence():
    ...
    def load_data(self, sample_ids_file, sequences_file):
        ...
        if len(missing)>0:
            raise KeyError("Provided with sample_ids which are not in seqs.  There are {0} such sequences.  Examples are: {1}".format(len(missing),missing))
...
class VariantMatrix:
    ...
    def get_missingness_cutoff(self, positions: Set[int], mmodel: dict) -> int:
        ...
        median_missingness = np.median(missingness)         # study median, as this will be relatively insensitive to samples with high missingness
    ...
    def build(self, min_variant_freq=None, num_train_on = None, deterministic = True):
        ...
        logging.info("Found {0} positions which vary at frequencies more than {1}.".format(len(select_positions),min_variant_freq))
        ...
        logging.info(f"Removed {num_removed} positions with missingness > cutoff {upper_cutoff} sequences.  Represents removal of {pc_removed} %")
        logging.info(f"There remain {len(select_positions)} positions which vary at frequencies more than {min_variant_freq} and pass the missingness cutoff.")
        ...
        logging.info("Scanning for samples with unexpectedly high missingness (N), or likely to be mixed (M) in variation model")
        if self.show_bar:
        ...
        mix_quality_cutoff = 0.01 / len(mix_quality_info.index)     # 0.01 divided by the number of samples analysed;  Bonferroni adj.     
        ...
        logging.info("Identified {0} ({1}%) sequences with higher N/M in variant vs. non-variant bases (composition p cutoff {2}); excluded from model as may be mixed.".format(
        ...
        vmodel = {}
        nLoaded = 0
        logging.info("Gathering variation for matrix construction from {0} unmixed samples into dictionary ".format(len(guids_analysed_stage2)))
