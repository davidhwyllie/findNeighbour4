...
from sqlalchemy import event, Integer, Boolean, Column, Float, Index, Integer, MetaData, Table, Text, String, DateTime, Date, Identity
import sqlalchemy.engine.url
from sqlalchemy import create_engine, inspect, func
from sqlalchemy.orm import sessionmaker
from sqlalchemy.orm import relationship, backref, exc as orm_exc
from sqlalchemy import ForeignKey, Index
from sqlalchemy import exc  # exceptions
from sqlalchemy.ext.declarative import declarative_base
import cx_Oracle
from pca.pca import VariationModel, PCA
...
class StatisticalModel(db_pc):
    ...
    tcc_int_id = Column(Integer, ForeignKey(TransformedCoordinateCategory.tcc_int_id), index=True)
...
class StatisticalModelFit(db_pc):
    ...
    statmodel_int_id = Column(Integer, ForeignKey(StatisticalModel.statmodel_int_id), index=True)
    analysis_family_id  = Column(String(38))
    analysis_id = Column(String(38)) 
...
Index('ix_sequence_ids', 
            SequenceFeature.variable, 
...
class ContingencyTable():
    ...
    def __init__(self, build_int_id, pc_cat, sequencefeature, a, b, c, d):
        ...
        self.results = {'a':int(a), 'b':int(b), 'c':int(c), 'd':int(d), 'build_int_id':build_int_id, 'pc_cat':pc_cat, 'sequencefeature':sequencefeature}
...
class PCADatabaseManager():
    ...
    def __init__(self,
        ...
        print("PCADatabaseManager: Database connection made; there are {0} tables.  Oracle database = {1}".format(len(self._table_names()), self.is_oracle))
    ...
    def _drop_existing_tables(self):
        ...
        if remaining > 0:
            raise PCADBManagerError("Failed to remove all tables in the database.  The following remain: {0}".format(self._table_names()))
        return
    ...
    def _bulk_load(self, upload_df, target_table, max_batch = 100000):
        ...
        if not target_table in self._table_names():
            raise ValueError("target_table {0} does not exist: valid tables are {1}".format(target_table, self._table_names()))
        ...
        if self.is_sqlite:
            ...
            print("Autoset max_batch to {0}, as running SQLite".format(max_batch))
        ...
        if self.is_oracle:
            ...
            sql_statement = "INSERT INTO {0} ({1}) VALUES ({2})".format(target_table, target_cols, insert_cols)
            with cx_Oracle.connect(user = u, password = p, dsn = dsn) as conn:
                cursor = conn.cursor()

                while len(loadvar)>0:
                    print("Bulk upload of {0} : {1} remain".format(target_table, len(loadvar)))
            ...
            while len(upload_df.index)>0:
                print("Bulk upload of {0} : {1} remain using pandas".format(target_table, len(upload_df)))
                to_upload = upload_df.head(n = max_batch)
                to_upload.to_sql(target_table, self.engine, if_exists='append', index=False, method = 'multi')
    ...
    def store_variation_model(self, vm):
        ...
        if not isinstance(vm, VariationModel):
            raise TypeError("Need to be passed a VariationModel, not a {0]".format(type(VariationModel)))
        
        print("Creating new build.")
        this_build = Build(builder = "pcadb.PCADatabaseManager.store_variation_model",
        ...
        for key in vm.model:
            ...
            if native_type in [bool, int, float, str]:
                metadata.append({"variable": key, "value": str(vm.model[key]), "native_type": str(native_type)})
        ...
        for item in metadata:
            this_build.build_annotations.append(
                BuildAnnotation(**item)
            )
        # add base positions
        print("Loading base positions")
        for item in vm.model['contributing_basepos']:
            this_build.contributing_basepos.append(
                ContributingBasePos(basepos = item)
            )
        # add positions
        print("Loading contributing positions")
        for item in vm.model['contributing_pos']:
            this_build.contributing_pos.append(
                ContributingPos(pos = item)
            )
        # add explained variance ratio and positions per pc
        print("Loading explained variance ratio")
        ...
        print("Committing initial model data")  
        self.session.commit()
        ...
        print("Loading transformed_coordinate_categories")
        ...
        sample_df['suspect_quality'] = sample_df.index.isin(vm.model['suspect_quality_seqs'].index)
        ...
        get_sample_int_ids_sql = self.session.query(Sample.sample_id, Sample.sample_int_id).\
                            filter(Sample.build_int_id == this_build.build_int_id).statement
    ...
    def existing_sample_ids_in_clinical_metadata(self):
        ...
        existing_sample_ids = [x for x, in self.session.query(ClinicalMetadata.sample_id)]
        return(set(existing_sample_ids))

    def store_cog_metadata(self, cogfile, date_end = datetime.datetime.now(), replace_all = False):
        ...
        for ix in cogdf.index:
            if cogdf.at[ix, 'sample_id'] not in existing_sample_ids:
                if datetime.datetime.fromisoformat(cogdf.at[ix, 'sample_date']) <= date_end:
                    ...
                    sample_date = datetime.datetime.fromisoformat(cogdf.at[ix, 'sample_date']),
                    ...
                    lm = dict(
                            ...
                            sequencefeature = "{0}:{1}".format('pangolearn',cogdf.at[ix, 'lineage']),
                            is_lineage = True,
                            evidence_for =1,
                            evidence_against =0
                    )
                    ...
                    # add the mutations
                    mutations = ['e484k', 't1001i', 'd614g', 'p323l', 'del_21765_6', 'a222v', 'p681h', 'q27stop', 'n501y', 'n439k', 'y453f', 'del_1605_3']
                    for mutation in mutations:
                        sm_to_insert.append(
                            dict(
                                ...
                                sequencefeature = "{0}:{1}".format(mutation,cogdf.at[ix, mutation]),
                                is_lineage = False,
                                evidence_for =1,
                                evidence_against =0,
                                version = '-'                       
                    ...
                    sm = dict(
                            ...
                            sequencefeature = "{0}:{1}".format('scorpio',cogdf.at[ix, 'scorpio_call']),
                            is_lineage = True,
                            evidence_for =1,
                            evidence_against =0
                    )
        ...
        get_cm_int_ids_sql = self.session.query(ClinicalMetadata.sample_id, ClinicalMetadata.cm_int_id).statement
    ...
    def store_cog_metadata_old(self, cogfile, date_end = datetime.datetime.now(), replace_all = False):
        """ extracts data from cog-uk format metadata files and imports them into RDBMS
        ...
        for ix in cogdf.index:
            if cogdf.at[ix, 'sample_id'] not in existing_sample_ids:
                if datetime.datetime.fromisoformat(cogdf.at[ix, 'sample_date']) <= date_end:
                    ...
                    if n_added % 2500 == 0:
                        print("Committing sequence metadata ",n_added)
    ...
    def sequence_features(self, is_lineage):
        ...
        retVal = [x for x, in self.session.query(SequenceFeature.sequencefeature).\
                                    filter(SequenceFeature.sequencefeature != None).\
    ...
    def make_contingency_tables(self, only_pc_cats_less_than_days_old = 120):
        ...
        a_b_c_d,  = self.session.query(func.count(Sample.sample_int_id)).\
            join(ClinicalMetadata, Sample.sample_id == ClinicalMetadata.sample_id).\
            filter(Sample.build_int_id == lbii).\
            filter(Sample.used_in_pca ==1).one_or_none()
        a_c_sql = self.session.query(
            ...
            join(ClinicalMetadata, SequenceFeature.cm_int_id == ClinicalMetadata.cm_int_id).\
            join(Sample, Sample.sample_id == ClinicalMetadata.sample_id).\
            filter(Sample.build_int_id == lbii).\
            filter(Sample.used_in_pca == 1).\
            filter(SequenceFeature.is_lineage==1).\
            group_by(SequenceFeature.sequencefeature).statement
        ...
        a_b_sql = self.session.query(
            TransformedCoordinateCategory.pc_cat,
            func.count(Sample.sample_int_id.distinct()).label('a_b'),
            func.min(ClinicalMetadata.sample_date).label('earliest_sample_date')).\
            join(Sample, TransformedCoordinateCategory.sample_int_id == Sample.sample_int_id).\
        ...
        print("Computing associations for {1} pc_cats, from which results for will be extracted for {0} pairs, or {2} comparisons".format(len(a_c.index), len(a_b.index), len(a_b.index)* len(a_c.index)))
        n_added = 0
        for a_b_ix in a_b.index:
            this_pc_cat = a_b.at[a_b_ix, 'pc_cat']
            print("Computing associations for {0}".format(this_pc_cat))
            ...
            for a_c_ix in a_c.index:
                ...
                res = self.session.query( 
                    ...
                    join(ClinicalMetadata, ClinicalMetadata.sample_id ==  Sample.sample_id).\
                    join(SequenceFeature, SequenceFeature.cm_int_id == ClinicalMetadata.cm_int_id).\
                    join(TransformedCoordinateCategory, TransformedCoordinateCategory.sample_int_id == Sample.sample_int_id).\
                    filter(Sample.build_int_id == lbii).\
                    filter(Sample.used_in_pca == 1).\
                    filter(SequenceFeature.is_lineage == 1).\
                    filter(TransformedCoordinateCategory.pc_cat == this_pc_cat).\
                    filter(SequenceFeature.sequencefeature == this_sequencefeature).\
                    group_by(TransformedCoordinateCategory.pc_cat, SequenceFeature.sequencefeature).one_or_none()
                if res is None:
                    this_a = 0
                else:
                    this_a = res[0]
                ...
                n_added +=1
                ct = ContingencyTable(lbii, this_pc_cat, this_sequencefeature, this_a, this_b, this_c, this_d)
