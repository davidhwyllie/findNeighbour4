...
Four classes provide the functionality:
...
class ModelBuilder():
    ...
    def build(self, n_components='mle', min_variant_freq=None, train_on = None):
        ...
        # set minimum variation as 1/ (0.1* number of samples).  Corresponds to a variant having to be seen 10 times in the dataset to be included.
        ...
        print(">> Discarded {0} sequences based on their having unexpectedly higher Ns in variant vs. non-variant bases".format(len(set(suspect_quality.index.to_list()))))
...
class pcaComparer():    
    ...
    * compare and mcompare - compare pairs, or all, samples, respectively.
    ...
    def __init__(self, model, what = 'ACTG', computational_method = 1):
        ...
        input:
            ...
            computational_method: 
                    1 = compute differences of eigenvalues
                    2 = compute ratio of eigenvalues, and normalise to median of ratios (method of QC)
        """
        self.vm = model
    ...
    def _compress_one(self, guid, obj):
        ...
        if len(set(obj.keys()) - self.compressed_sequence_keys) > 0:
            raise TypeError("obj has keys in {0} but should have keys {1}".format(obj.keys(), self.compressed_sequence_keys))
    ...
    def compress(self, guid, obj, enable_bar=True):
        ...
        if use_bar:
            print("mcompress: extracting data ..")
            bar = progressbar.ProgressBar(max_value = len(guids))
        ...
        if use_bar:
            print("mcompress: Computing eigenvalues .. preparing matrix ..")
        ...
        if use_bar:
            print("mcompress: Computing eigenvalues .. matrix multiplication ..")

        evs = self.vm.model['pca'].transform(vm_df)

        if use_bar:
            print("mcompress: Computing eigenvalues .. storing result ..")
    ...
    def mcompare(self, guid, guids=None):
        ...
        if self.computational_method ==1:
            target = self.pcaArray.loc[guids] - self.pcaArray.loc[guid]     # this is the difference between the components' scores.  
            # we are concerned with absolute deviations
            target = target.apply(abs)                      # we are concerned with fold change from the median

                                                # Formerly took logs and subtracted the median.  This latter is a method of controlling for assay level variability, but if such variation is substantial
                                                # it will prevent us using k-means approaches later
        else:
            # compute ratios and normalise
        
            target = self.pcaArray.loc[guids] / self.pcaArray.loc[guid]
            medians = target.median(axis=1, skipna=True)
            target = target.sub(medians,axis=0)
            target = target.apply(abs)
...
# check input
        ...
        for item in what:
            ...
            self.pcc[item] = pcaComparer(model= model, what = item, computational_method = 1)


    def validate(self, sample_size=30, internal= False,snp_cutoff=12, pca_cutoff=None, compare_with_SNV=True):
        ...
        print("Computing eigenvalues for  guids")
            ...
            for guid2 in guids_to_study:
                ...
                if guid1 < guid2:   #half matrix only
                    ...
                    for item in self.what: 
                        ...
                        qs2 = self.pcc[item].qualityScoring.loc[guid2].to_dict()
                        for key in qs1:
                            add_dict["guid1_"+key]=qs1[key]
                            add_dict["guid2_"+key]=qs2[key] 

                    comparisons.append( add_dict) 
                    #print(add_dict['snp_dist'], add_dict['pca_dist_ACTG']) 
                    bar.update(nLoaded)
...
""")
    ...
    model = builder.build(train_on=500, n_components=20)
    what = ['ACTG']     #
    compare_with_SNV = True
    pcv = pcaValidator(CONFIG= CONFIG, PERSIST=PERSIST, model=model, what = what    )
    
    pca_ceiling = pcv.validate(sample_size=10, internal=True, compare_with_SNV=compare_with_SNV)
    for item in what:
        filename  = "v14_explained_variance_{0}.csv".format(item)
        pcv.pcc[item].vm.model['explained_variance'].to_csv(filename)
    ...
    if compare_with_SNV:
        for item in what:
            ...
            filename  = "v14_internal_sampling_output.csv"
            pcv.validation_data.to_csv(filename)
            print("Exported ",filename)
            print("PCA cutoff was ",pca_ceiling)
